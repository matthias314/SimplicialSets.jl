...
otherwise `T` is assumed to be a multiplicative discrete group.

The constructor accepts an iterator over the components of the bar simplex.
The optional keyword `op` indicates whether the group is multiplicative (default) or additive.
...
Return `true` if `x` is the identity element in the group of bar simplices of dimension
