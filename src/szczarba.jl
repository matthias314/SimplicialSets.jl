...
function lefttwistedproductsimplex(twf)
    LinearExtension(splat(Fix1(LeftTwistedProductSimplex, twf)))
end
...
function righttwistedproductsimplex(twf)
    LinearExtension(splat(Fix1(RightTwistedProductSimplex, twf)))
end

keeps_filtered(::Base.Splat{<:Fix1{<:Type{<:Union{LeftTwistedProductSimplex,RightTwistedProductSimplex}}}}, ::Type) = true
...
show(io::IO, sz::SzczarbaTwc) = print(io, "SzczarbaTwc(", sz.twf, ')')

@struct_equal_hash SzczarbaTwc
...
LinearCombinations.return_type(::typeof(diff), ::Type{T}) where T <: LinearCombinations.AbstractTwistedTensor{<:AbstractSimplex,<:AbstractSimplex,<:SzczarbaTwc} =
    Linear{T,Int}
...
show(io::IO, szsh::SzczarbaShuffle) = print(io, "SzczarbaShuffle(", szsh.twf, ')')

@struct_equal_hash SzczarbaShuffle
...
LinearCombinations.return_type(::typeof(coprod), ::Type{T}) where T <: RightTwistedTensor{<:AbstractSimplex, <:AbstractSimplex, <:SzczarbaTwc} =
    Linear{Tensor{Tuple{T, T}}, Int}
